...
// Bypasses any browser/CDN cache for manual refresh clicks
const forceFetcher = (url: string): Promise<FeedResult> =>
...
export default function MainDashboard() {
  ...
  // Hide the splash once the very first fetch resolves (data or error)
  useEffect(() => {
  ...
  const SPLASH_SOURCES = ["YouTube Shorts", "Pexels Photos", "Reddit Trends", "NewsData", "AI Synthesis"];

  return (
    <>
      {/* ── Full-screen client-side splash (shows on every first load / Vercel) */}
      <AnimatePresence>
        {showSplash && (
          <motion.div
            key="splash"
          ...
          >
            ...
            <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden">
              ...
              <div className="absolute right-0 top-1/3 h-64 w-64 rounded-full bg-violet-700/10 blur-3xl" />
    ...
    <div className="min-h-screen bg-[#050505] text-white">
      ...
      <main className="relative z-10 mx-auto max-w-screen-xl px-4 pb-16 pt-6">
        ...
        <Toolbar
          ...
          isLoading={isLoading}
          onRefresh={() => {
            // Refetch with cache bypassed — keeps existing cards visible while loading
            void mutate(forceFetcher(swrKey), { revalidate: false });
          }}
        />
        ...
        {/* Error */}
        {!isLoading && error && (
          <EmptyState onRetry={() => void mutate(forceFetcher(swrKey), { revalidate: false })} />
        )}
